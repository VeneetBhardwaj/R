...
This project on making a movie recommendation system using R, with the *movielens* data.
...
The approach used by these system can be a collaborative approach and/or content-based approach

### Collaborative Approach

  This filtering is based on the user's past behavior and similar choices made by other users. These data points are then used to predict the items/ratings for items that the user may have interest in. This method does not rely on the knowledge about the item itself. 
...
### Content-based Approach
  ...
  This filtering uses a set of discrete characteristics of an item in order to recommend items with similar properties. Thus this method needs little information to start with but are limited to the information on the item itself.
...
The above code is as provided by edx for initialization. Due to the time it takes, and sometimes multiple attempts required to download the accurate data set a backup of the environment, with the 2 data tables edx and validation, has been created.
These are being loaded for the use in this project. (NOTE : All the codes will work and give the same results if the code snippet above is run and the backup ignored.)

```{r}
#Backup data is in the same folder as the rmd file being presently run
load("backup.RData")
```
...
+   **movieId** is the unoque movie identification
...
This does make it a bit difficult to recommend a movie given no one has rated it. After all there is nothing else that we know about the movie itself.

Sparsity of the data set visualized below using a matrix of a random sample of 100 movies and 100 users with colored indicating a user/movie combination for which there is a rating.
...
Sorting the movies and users by the number of ratings.
...
These charts might be representing something of the blockbuster effect of the movies itself. Highly rated movies have a higher entertainment value and thus get rated more and higher!
...
Recall that some movies have been categorized into multiple genres separated by |.
...
+   mean of the rating might be higher for genres with very few movies in them.
...
An approach to solving this problem is using the similarity matrix or the distance measure. This approach solves the problem of ColdStart, Scalabilty and Sparsity thus improves prediction accuracy.
...
Given the limitation of the low hardware/data ratio the similarity is calculated for 500 users and 500 movies. 
...
To decide on an effective $\lambda$, the tuning parameter, cross validation is used.
...
```{r message=FALSE, warning=FALSE}
model_5_rmse <- rmses[lambda]
rmse_results <- bind_rows(
  rmse_results,data_frame(method="Movie + User + AgeofMovie Regularised Model",
                          RMSE = model_5_rmse ))
rmse_results
```
...
+   POPULAR which it based on Item (in this case movies ) popularity.
...
+   All the three models predict different movies for the same user. This might be attributed to the limited amount of data that we are providing for learning.

Given the limitation of the low hardware/data ratio running of the recommender using the entire data set is not possible. Thus, for the purpose of illustration, the upper and lower 90% quantiles of the data are being ignored. This will result in most of the information being there in the matrix that is used for learning and testing.
...
Thus the validation_f and the edx_f as the sets for the final validation of the model.

\newpage

## Regularized Linear Model 

In the testing process, the linear model with regularization on the users and the movies effects achieved the required project RMSE objective. The same model is now run on the complete edx set with the validation set being used for predict.
...
```{r message=FALSE, warning=FALSE}
rmse_results <- bind_rows(
  rmse_results,data_frame(method="Final: Movie + User + AgeofMovie Regularised Model (validation)",
                          RMSE = Final_rmse ))
rmse_results
```
...
opts <- r$tune(train_data, opts=list(dim = c(10,20,30),lrate = c(0.1,0.2),
                                     costp_l1=0, costq_l1=0,
                                     nthread= 2, niter=10, verbose=FALSE))
```

```{r recosystemresult2, message=FALSE, warning=FALSE, cache=TRUE}
#train the model
r$train(train_data, opts = c(opts$min, nthread=2, niter=77, verbose = FALSE))
#predicts the results
y_hat <- r$predict(test_data, out_memory())
```
...
(For illustration, and curiosity, only!)
...
In this project on building a recommendation system for the movielens data set, with the 10M version, the objective of **RMSE less than 0.8649**
This was achieved.
